...
void AMS_AS5048B::setI2CAddress(uint8_t chipAddress)
{
	_chipAddress = chipAddress;
	_debugFlag = false;
}
...
uint8_t AMS_AS5048B::addressRegR(void) {
	...
	return AMS_AS5048B::readReg8(AS5048B_ADDR_REG) ; // set all unused bits to 0
}
...
uint16_t AMS_AS5048B::readReg16(uint8_t address) {
	...
	_bus->write(address);
	requestResult = _bus->endTransmission(false);
	_bus->requestFrom(_chipAddress, (size_t)2,I2C_STOP,0);
